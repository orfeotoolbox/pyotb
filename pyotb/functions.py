...
def where(cond, x, y):
    ...
    if cond_nb_channels == 1 and x_or_y_nb_channels and x_or_y_nb_channels != 1:
        logger.info('The condition has one channel whereas X/Y has/have {} channels. Expanding number of channels '.format(x_or_y_nb_channels) +
...
def define_processing_area(*args, window_rule='intersection', pixel_size_rule='minimal', interpolator='nn',
    ...
    if not all(metadata['UpperLeftCorner'] == any_metadata['UpperLeftCorner']
        ...
        logger.info('Cropping all images to extent Upper Left ({}, {}), Lower Right ({}, {})'.format(ULX, ULY, LRX, LRY))
    ...
    if not all(metadata['GeoTransform'][1] == any_metadata['GeoTransform'][1]
        ...
        logger.info('Resampling all inputs to resolution : {}'.format(pixel_size))
