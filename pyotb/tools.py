...
def find_otb(root="", scan=True, scan_userdir=True):
    ...
    if prefix:
        logger.info("Found OTB Python API in {}".format(prefix))
        ...
        if "otb" not in globals():
            try:
                ...
                logging.critical("Can't import OTB with OTB_ROOT={}".format(prefix))
                return None, None
        else:
    ...
    # Else search system
        ...
        logger.info("Failed to import otbApplication with PYTHONPATH={}".format(PYTHONPATH))
        if not scan:
            return None, None
        ...
        if scan_userdir :
            for path in Path().home().glob("**/OTB-*/lib"):
                logger.info("Found {}".format(path.parent))
                lib_dir = path
...
def get_apps_path(lib_dir):
    if isinstance(lib_dir, Path) and lib_dir.exists():
        logger.debug("Using library from {}".format(lib_dir))
        ...
        logger.debug("Library directory found but no 'applications' directory whithin it")
        return ""
...
def set_gdal_vars(root):
    if (Path(root) / "share/gdal").exists():
        ...
        logger.warning("Can't find GDAL directory with prefix {}".format(root))
        return False
