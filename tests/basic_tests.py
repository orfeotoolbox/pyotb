...
# Test ReadImageInfo
info = pyotb.ReadImageInfo(inp, otb_stdout=False)
assert info.sizex == 251
assert info.sizey == 304
assert info['numberbands'] == info.numberbands == 4

# Test Statistics
stats = pyotb.ComputeImagesStatistics(il=inp, otb_stdout=False)
assert stats['out.min'] == "[33, 64, 91, 47]"

# Test Statistics on a Slicer
slicer_stats = pyotb.ComputeImagesStatistics(il=[inp[:10, :10, 0]], otb_stdout=False)
